...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    ...
    <div class="flex gap-2">
      ...
      <a href="{{ url('/user/sales/'.$sale->id.'/receipt') }}" target="_blank" class="px-4 py-2 bg-indigo-600 text-white rounded">Lihat Struk</a>
      {{-- Tidak ada tombol Refund di sisi user --}}
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
    ...
    <div class="p-4 bg-white rounded shadow">
      ...
      <div class="text-xl font-semibold text-red-600">- Rp {{ number_format($sale->calc_auto_discount,0,',','.') }}</div>
    </div>
    <div class="p-4 bg-white rounded shadow">
      <div class="text-sm text-gray-500">Diskon Voucher</div>
      <div class="text-xl font-semibold text-red-600">- Rp {{ number_format($sale->calc_voucher_discount,0,',','.') }}</div>
    </div>
    <div class="p-4 bg-white rounded shadow">
      <div class="text-sm text-gray-500">Grand Total</div>
      <div class="text-xl font-semibold">Rp {{ number_format($sale->calc_grand_total,0,',','.') }}</div>
      <div class="text-xs text-gray-500">Bayar: Rp {{ number_format($sale->paid_amount,0,',','.') }} • Kembali: Rp {{ number_format($sale->change_amount,0,',','.') }}</div>
    </div>
  </div>
  ...
  <div class="bg-white rounded shadow overflow-x-auto">
    <table class="min-w-full text-sm">
      ...
      <tfoot>
        <tr><td colspan="3" class="px-4 py-2 text-right">Subtotal</td><td class="px-4 py-2 text-right">Rp {{ number_format($sale->calc_subtotal,0,',','.') }}</td></tr>
        <tr><td colspan="3" class="px-4 py-2 text-right">Diskon Otomatis</td><td class="px-4 py-2 text-right text-red-600">- Rp {{ number_format($sale->calc_auto_discount,0,',','.') }}</td></tr>
        <tr><td colspan="3" class="px-4 py-2 text-right">Diskon Voucher</td><td class="px-4 py-2 text-right text-red-600">- Rp {{ number_format($sale->calc_voucher_discount,0,',','.') }}</td></tr>
        <tr><td colspan="3" class="px-4 py-2 text-right">Grand Total</td><td class="px-4 py-2 text-right font-semibold">Rp {{ number_format($sale->calc_grand_total,0,',','.') }}</td></tr>
      </tfoot>
    </table>
  </div>
  ...
  <div class="mt-6 bg-white rounded shadow">
    <div class="px-4 py-3 border-b"><h3 class="font-semibold">Riwayat Refund / Tukar</h3></div>
            ...
            <td class="px-4 py-2">
              <ul class="list-disc pl-5">
                @foreach($r->items as $it)
                  <li>{{ $it->nama_barang }} — Kode {{ $it->kode_barang }}, Qty {{ $it->qty_refund }}, <em>{{ $it->condition==='damaged'?'rusak':'normal' }}</em></li>
                @endforeach
              </ul>
              @if($r->notes)
